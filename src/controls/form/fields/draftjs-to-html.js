...
This is old function and returns HTML 4 nested list
*/
...
*/
export default function draftToHtml(editorContent, hashtagConfig, directional, customEntityTransform, overrideFor) {
  var html = [];
      ...
      blocks.forEach(function (block) {
        if (isList(block.type)) {
          ...
          if (listBlocks.length > 0) {
            var listHtml = {};
            listHtml = getListMarkup(listBlocks, entityMap, hashtagConfig, customEntityTransform); // eslint-disable-line max-len
            html.push(listHtml);
            listBlocks = [];
          }
...
const getListMarkup=(listBlocks, entityMap, hashtagConfig, directional, customEntityTransform, sameLevelParentFound=false)=> {
  var listHtml = [];
  var nestedListBlock = [];
  var previousBlock = null;
  listBlocks.forEach(function (block) {
    var nestedBlock = false;
    var blockIndex=listBlocks.findIndex(({key})=>key===block.key);
    var nextBlock = listBlocks.length >blockIndex ?  listBlocks[blockIndex+1]:null;
    if (!previousBlock) {
      listHtml.push('<' + getBlockTag(block.type) + '>\n');
    } else if (previousBlock.type !== block.type) {
      listHtml.push('</' + getBlockTag(previousBlock.type) + '>\n');
      ...
      if (nestedListBlock && nestedListBlock.length > 0) {
        listHtml.push(getListMarkup(nestedListBlock, entityMap, hashtagConfig, directional, customEntityTransform, true));
        nestedListBlock = [];
      }
    } else {
      nestedBlock = true;
